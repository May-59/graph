...
class ConvE(nn.Module):  
    def __init__(self, model_name, nentity, nrelation, args):  
        ...
        self.fc = nn.Linear(self.out_channels * (self.embedding_dim - self.kernel_size + 1), self.hidden_dim)  
    ...
    def forward(self, px, nx, py, ny):  
        # px, nx: (batch_size, 3) [head_index, relation_index, tail_index]  
        # py, ny: (batch_size, neg_ratio, 3) [head_index, relation_index, tail_index]  
  
        # extract embeddings of head, relation, and tail entities  
        head_emb = self.entity_embedding(px[:, 0])  
        relation_emb = self.relation_embedding(px[:, 1])  
        tail_emb = self.entity_embedding(px[:, 2])  
  
        # stack embeddings of head, relation, and tail entities  
        stacked_inputs = torch.stack([head_emb, relation_emb, tail_emb], dim=1)  
        reshaped_inputs = stacked_inputs.view(-1, 1, 3, self.embedding_dim)  
        
        # convolutional layer  
        conved = F.relu(self.conv(reshaped_inputs))  
        conved_flat = conved.view(-1, self.out_channels * (self.embedding_dim - self.kernel_size + 1))  
  
        # Linear layer  
        scores = self.fc(conved_flat)  
        positive_score = self.gamma.item() - torch.norm(scores, p=1, dim=1, keepdim=True)  
  
       
  
        return self.loss(positive_score, None)  
  
    def loss(self, positive_score, negative_score=None):  
